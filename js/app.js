...
(function () {
  ...
  //`${window.location.origin}/host?room=${generateId()}`
  //);
  let bgChoice = null;
  /**
  ...
  //const c = JSON.parse(el.getAttribute('data'));
  //el.remove();
  ...
  const updateStatus = function (session, status) {
    ...
    if (status === 'active') {
      ...
      displayUrl.innerHTML = window.location.href.replace(
        'host?',
        'hls-viewer?'
      );
  ...
  const startBroadcast = function () {
    ...
    if (bgChoice !== null) {
      startExperienceComposer(
        '2_MX40NzU4NDg2MX5-MTY4NjM2NjUxMzMzN35ZWmNZbGw4NUlBcUc2Z3ViNG5WZmorckV-fn4',
        roomName,
        bgChoice,
        roundyes
      );
  ...
  const init = function () {
    ...
    session.connect(
      'T1==',
      function (error) {
        if (error) {
          console.log(error);
